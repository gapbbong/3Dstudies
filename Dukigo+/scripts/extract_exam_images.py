...
DASHBOARD_FILE = os.path.join(BASE_DIR, "scripts", "mapping_dashboard.html")
...
def ask_directories():
    ...
    print("\n[Step 1] 시험지 폴더(PDF 포함)를 선택해 주세요...")
    scan_dir = filedialog.askdirectory(title="시험지 원본 폴더 선택 (PDF 포함)")
    if not scan_dir: return None, None
    
    print("\n[Step 2] 문제 데이터 폴더(JSON 포함)를 선택해 주세요...")
    data_dir = filedialog.askdirectory(title="문제 데이터 폴더 선택 (JSON 포함)", initialdir=scan_dir)
    if not data_dir: return None, None
    
    root.destroy()
    return scan_dir, data_dir
...
<head>
    <meta charset="utf-8">
    <title>Dukigo Real Explorer v6.2.1</title>
    <style>
...
<body>
    <div id="sidebar">
        <div class="header"><h2>📂 Dukigo Real Explorer v6.2.1</h2></div>
...
def main():
    print("Project: Dukigo Full Explorer v6.2.1")
    ...
    print("\nStep 2: Preparing v6.2.1 Explorer data with Parent Navigation...")
    dashboard_data = prepare_dashboard_data(scan_root, data_root, all_images_info)
    print("\nStep 3: Generating v6.2.1 Dashboard...")
    generate_dashboard_html(dashboard_data, DASHBOARD_FILE)
    print(f"\nv6.2.1 Dashboard ready: {DASHBOARD_FILE}")
